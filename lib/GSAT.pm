...
=head1 graphClip
    ...
    -minPathNum [int]  the min number of supported paths for remaining a link.
...
=head1 graphMatrix
    ...
    -fileList|-f [str]     the list file containing sample name, *.mapping.paths and *.gfa for each sample in a line.*
    -contigList|-c [str]   the list file containing the names of pangenomic segments across those graphs.*
    -out|-o [str]          the name prefix of output files.
...
sub pipeCorr{
   ...
   #my $memory_before = graphCorrector::memory_usage();
   #print "进程内存-sxx00: ${memory_before}KB\n";
   
   run_in_subprocess('graphMapper::gmap',\@gmap_opt);
   
   #$memory_before = graphCorrector::memory_usage();
   #print "进程内存-sxx01: ${memory_before}KB\n";
...
sub pipeSim{
   ...
   if(defined($auto_est)){
      ...
      my $ratio_adj=0.3;#相对于top_depth的比例
      $minPath = int($ratio_adj * $nd50);
      $minAddNo = $minPath;
